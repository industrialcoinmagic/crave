...
{
    // Convert crave:// to crave:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("crave://"))
    {
        uri.replace(0, 12, "crave:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Crave.lnk";
}
...
{
    return GetAutostartDir() / "crave.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Crave-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  crave-qt [" + tr("command-line options") + "]                     " + "\n";
